...
# 3: Creating a New Data Frame with Relevant Variables
...
Note: All models are logistic regression models with spending habits as the response variable.

### Model 1: Materialism as the predictor variable
...
### Model 2: Impulsivity/self-control as the predictor variable
...
### Model 3: materialism AND impulsivity as the predictor variables
...
### Model 4: Materialism & impulsivity AND both variables as an interaction term
...
thoughts: the impulsive model seems to have the best fit. stronger p values too.

choices: #2 for simplicity and lowest bic, #3 for maximisation of model fit but will need to explain p values
...
\*Note: I'm pretty sure I said this the other way around in the presentation, that Model 3 had a lower BIC than Model 2. I think I did. If so... that was yet another error on my part.
...
-   A logistic model doesn't assume a linear relationship between the dependent and independent variables.
    ...
    -   Should I investigate the log odds using logit? Probably. ¯\\\_(ツ)\_/¯ (Acknowledging that I didn't - AR)
...
ggplot(grid, aes(x = materialism, y = predicted_prob, color = impulsive, group = impulsive)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  scale_color_brewer(palette = "Paired") +
  labs(
    title = "Predicted Probability of Good Spending Habits",
    x = "Materialism Scale (from low to high)",
...
The results from the graph shows us that self control and materialism does affect people's spending habits, at least to some extent. Impulsivity/self-control more than materialism (see: the gaps between the lines), which is also shown by our other models and their results.
